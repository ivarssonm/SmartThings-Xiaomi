 ...
 *  -----------------
 *  GvnCampbell - Changes added to support my Xiaomi Aqara Door/Window Sensor (Model MCCGQ11LM)
...
private Map parseCatchAllMessage(String description) {
	def linkText = getLinkText(device) + ' [parseCatchAllMessage]'
	log.debug "${linkText}: '${description}'"
	...
	if (cluster) {
		switch(cluster.clusterId) {
			...
			//	break
            default:
            	log.debug "${linkText}: clusterId unknown"
            	break
		}
	}

	log.debug "${linkText}: Returning '${resultMap}'"
	return resultMap
}

private boolean shouldProcessMessage(cluster) {
	def linkText = getLinkText(device) + ' [shouldProcessMessage]'
	log.debug "${linkText}: '${cluster}'"
...
def configure() {
    def linkText = getLinkText(device) + ' [configure]'
	log.debug "${linkText}: Started..."
...
def enrollResponse() {
	def linkText = getLinkText(device) + ' [enrollResponse]'
...
def refresh() {
	def linkText = getLinkText(device) + ' [refresh]'
	log.debug "${linkText}: Started..."
...
private Map parseCustomMessage(String description) {
	def linkText = getLinkText(device) + ' [parseCustomMessage]'
	log.debug "${linkText}: '${description}'"
...
private Map getContactResult(value) {
	def linkText = getLinkText(device) + ' [mapContactResult]'
	log.debug "${linkText}: '${value}'"
...
// Device wakes up every 1 hour, this interval allows us to miss one wakeup notification before marking offline
	def linkText = getLinkText(device) + ' [installed]'
	log.debug "${linkText}: Started ..."
...
// Device wakes up every 1 hours, this interval allows us to miss one wakeup notification before marking offline
    def linkText = getLinkText(device) + ' [updated]'
	log.debug "${linkText}: Started..."
